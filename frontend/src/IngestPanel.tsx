...
function IngestPanel() {
  ...
  const handleUpload = async (source: string, file: File) => {
    ...
    if (validationError) {
      setResults((prev) => ({ ...prev, [source]: { source, status: "error", message: validationError } }));
      return;
    }
    ...
    try {
      ...
      setResults((prev) => ({
        ...prev,
        [source]: { source, status: "ok", message: `inserted ${data.inserted}, updated ${data.updated}${driftNote}` },
      }));
      queryClient.invalidateQueries({ queryKey: ["active-users"] });
      queryClient.invalidateQueries({ queryKey: ["cost-by-tool"] });
      queryClient.invalidateQueries({ queryKey: ["events"] });
    } catch (err: any) {
      setResults((prev) => ({ ...prev, [source]: { source, status: "error", message: err.message ?? "upload failed" } }));
    } finally {
      setLoading(null);
    }
  };
    ...
    <section className="mb-12">
      ...
      <div className="grid grid-cols-3 gap-6">
        {SOURCES.map(({ key, label, accept }) => {
          ...
          return (
            <div key={key} className="border border-line p-5 hover:border-ink/30 transition-colors">
              <p className="text-sm font-medium mb-1">{label}</p>
              <p className="font-mono text-xs text-mute mb-4">accepts {accept}</p>

              <label className="block">
                <span className="inline-block font-mono text-xs border border-ink px-3 py-1.5 cursor-pointer hover:bg-ink hover:text-paper transition-colors">
                  {loading === key ? "uploading…" : "choose file"}
                </span>
                <input
                  type="file"
                  accept={accept}
              ...
              {fileName && (
                <p className="font-mono text-xs text-mute mt-2 truncate">{fileName}</p>
              )}
